...
public class ReportViewController2: UITableViewController {
    
    public final var measures: [TaskController]?
    ...
    public convenience init(_ measures: [TaskController], submitTo server: Server?, patient: Patient?) {
    ...
    func submitToEHR(_ sender: Any) {
        ...
        for report in selected_reports {
            group.enter()
            report.submit(to: server, patient: patient, request: nil) { (success, error) in
                if let error = error {
    ...
    override public func tableView(_ tableView: UITableView, accessoryButtonTappedForRowWith indexPath: IndexPath) {
        ...
        let bundleView = BundleViewController(gr.bundle)
